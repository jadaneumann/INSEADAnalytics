...
1. Yes: 0.64 for Q13 & Q14  /  0.63 for Q16 & Q17  /  0.62 for Q27 & Q28
   They seem to make sense, in that they are similar questions and represent similar behaviors
   Q13 = When buying a boat, I tend to buy the latest & greatest 
   Q14 = When buying accessories, I tend to buy the latest & greatest

2. It implies something about how the survey was designed (i.e. to capture similar traits using different questions / angles).  It also implies that certain personality traits / habits are likely able to be grouped.
...
1. The table and plot show the amount of the total data captured by the individual components (sorted in order from the component that captures the most data to the one that captures the least).  We can learn how much of the data we would "lose" by removing components.

2. The plot has this shape because it is sorted in order from the component that captures the most data to the one that captures the least.  An eigenvalue greater than 1 indicates that the component captures more than the average component.

3. We would prefer to see a few components that capture a large amount of data.  This would simplify our analysis and allow us to cut out many redundant components.
...
1. +Factor 1 relates to status, based on:
   Q1.9 = I see my boat as a status symbol
   Q1.26 = Having a powerful boat is what is most important to me
   Q1.3 = The brand says a lot about who I am
   + Factor 2 relates to recreation, based on:
   Q1.18 = Boating gives me a feeling of adventure
   Q1.21 = Boating helps me escape from everyday life and relax
   Q1.19 = Boating allows me to experience nature
   + Factor 3 relates to brand sensitivity
   + Factor 4 relates to knowledgeability
   + Factor 5 relates to price sensitivity
   
2. We would chose 5 factors
   + a) Statistical Justification: 5 components have eigenvalues greater than 1 (average)
   + b) Interpretation Justification: 2 or 3 components so that visualization and interpretation is made easier
   + c) Business Justification: 4 or 5 factors because each of the top 5 factors described in question 1 above seem to provide different customer preferences or business insights (i.e. little to no overlap).  Brand Sensitivity could arguably have some overlap with status, knowledgeability and/or price sensitivity.
   
3. The company has different customer types, for example some that care more about status, and some that care more about the actual performance of the boats

4. Adding domain knowledge to the raw data may help to produce a more meaningful model.
...
# Please use numbers, not column names, e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
segmentation_attributes_used = c(10,18,5,12,3) #c(10,19,5,12,3) 
...
1. The numbers in the last table are scaled measures of how much each segment differs from the population average for each question (characteristic).  The most informative numbers are the most extreme, in absolute terms.  So if one segment is significantly more (close to 1) or significantly less (close to -1) represented by a given characteristic, it can help differentiate that segment from the other segments.

2. Key Features:
* Segment 1: Most Stongly (0.42) Q17 "Likely to recommend that brand to others" & Most Strongly Not (-0.35) Q1.2 "Getting the lowest price is more important than the brand"
* Segment 2: Most Stongly (0.15) Q1.9 "I see my boat as a status symbol" & Most Strongly Not (-0.18) Q17 "Likely to recommend that brand to others"
* Segment 3: Most Stongly (0.23) Q1.11 "Likely to perform minor repairs on my own" & Most Strongly Not (-0.29) Q1.9 "I see my boat as a status symbol"
* Segment 4: Most Stongly (0.11) Q11 "Female" & Most Strongly Not (-0.43) Q1.11 "Likely to perform minor repairs on my own"
* Segment 5: Most Stongly (0.35) Q1.2 "Getting the lowest price is more important than the brand" & Most Strongly Not (-0.18) Q18 "Likely to buy the same brand in the future if you bought a new boat"
